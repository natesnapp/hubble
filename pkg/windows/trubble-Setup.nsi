;TrubbleStack Installer
;Install Trubble with Index, Indexer, and Token options
;Written by Chad Mills

;--------------------------------
!define PRODUCT_NAME "Trubble"
!define PRODUCT_NAME_OTHER "TrubbleStack"
!define PRODUCT_PUBLISHER "Adobe, Inc"
!define PRODUCT_WEB_SITE "https://trubblestack.io/"
!define PRODUCT_CALL_REGKEY "Software\Microsoft\Windows\CurrentVersion\App Paths\trubble.exe"
...
!ifdef TrubbleVersion
    !define PRODUCT_VERSION "${TrubbleVersion}"
!else
...
;General
  ...
  OutFile "Trubble-${PRODUCT_VERSION}-Setup.exe"
  
  ;Default Installation folder
  InstallDir "C:\${PFILES}\Trubble"
...
;Interface Settings
  ...
  !define MUI_ICON "trubble.ico"
  !define MUI_UNICON "trubble.ico"
...
;Pages
  ...
  ; Configure Trubble page
  Page custom pageTrubbleConfig pageTrubbleConfig_Leave
...
;Custom Dialog Box Variables
  Var Dialog
  Var Label
  Var CheckBox_Trubble_Start
  Var CheckBox_Trubble_Start_Delayed
  Var HECToken
  Var HECToken_State
  Var IndexName
  Var IndexName_State
  ...
  Var Indexer_State
  Var StartTrubble
  Var StartTrubbleDelayed
  Var DeleteInstallDir

;--------------------------------
;Trubble Settings Dialog Box

  Function pageTrubbleConfig

    # Set Page Title and Description
    !insertmacro MUI_HEADER_TEXT "Trubble Settings" "Set the Token and Index for Splunk (Click Next to Skip)"
  ...
  Function pageTrubbleConfig_Leave
...
;Custom Finish Page
  ...
  Function pageFinish_Show
    ...
    # Create Start Trubble Checkbox
    ${NSD_CreateCheckbox} 120u 90u 100% 12u "&Start Trubble"
    Pop $CheckBox_Trubble_Start
    SetCtlColors $CheckBox_Trubble_Start "" "ffffff"
    # This command required to bring the checkbox to the front
    System::Call "User32::SetWindowPos(i, i, i, i, i, i, i) b ($CheckBox_Trubble_Start, ${HWND_TOP}, 0, 0, 0, 0, ${SWP_NOSIZE}|${SWP_NOMOVE})"
	
	# Create Start Trubble Delayed Checkbox
	${NSD_CreateCheckbox} 130u 102u 100% 12u "&Delayed Start"
    Pop $CheckBox_Trubble_Start_Delayed
    SetCtlColors $CheckBox_Trubble_Start_Delayed "" "ffffff"
    # This command required to bring the checkbox to the front
    System::Call "User32::SetWindowPos(i, i, i, i, i, i, i) b ($CheckBox_Trubble_Start_Delayed, ${HWND_TOP}, 0, 0, 0, 0, ${SWP_NOSIZE}|${SWP_NOMOVE})"

    # Load current settings for Trubble
    ${If} $StartTrubble == 1
        ${NSD_Check} $CheckBox_Trubble_Start
    ${EndIf}
  ...
  Function pageFinish_Leave
    ...
    ${NSD_GetState} $CheckBox_Trubble_Start $StartTrubble
...
;Installer Sections
  ...
  Section "MainSection" SEC01
    ...
    CreateDirectory $INSTDIR\etc\trubble\trubble.d
    File /r "..\..\dist\trubble\"
  ...
  Section -Post
    ...
    WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" \
        "DisplayIcon" "$INSTDIR\trubble.ico"
    WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" \
    ...
    WriteRegStr HKLM "SYSTEM\CurrentControlSet\services\trubble" \
        "DependOnService" "nsi"
    ...
    WriteRegStr HKLM "${PRODUCT_CALL_REGKEY}" "" "$INSTDIR\trubble.exe"

    ; Register the Trubble Service
    nsExec::Exec "nssm.exe install Trubble $INSTDIR\trubble.exe"
    nsExec::Exec "nssm.exe set Trubble Description Open Source software for security compliance"
    nsExec::Exec "nssm.exe set Trubble Application $INSTDIR\PortableGit\git-cmd.exe"
    nsExec::Exec "nssm.exe set Trubble AppDirectory $INSTDIR"
    nsExec::Exec "nssm.exe set Trubble AppParameters trubble.exe -c .\etc\trubble\trubble.conf"
    nsExec::Exec "nssm.exe set Trubble Start SERVICE_AUTO_START"
  ...
  Section Uninstall
    ...
    Call un.uninstallTrubble

    ; Remove C:\Program Files\Trubble from the Path
    Push "C:\${PFILES}\Trubble"
...
;Uninstaller Section
  ...
  !macro uninstallTrubble un
  Function ${un}uninstallTrubble

    ; Make sure we're in the right directory
    ${If} $INSTDIR == "c:\salt\bin\Scripts"
      StrCpy $INSTDIR "C:\${PFILES}\Trubble"
    ${EndIf}

    ; Stop and Remove trubble service
    nsExec::Exec 'net stop trubble'
    nsExec::Exec 'sc delete trubble'
  ...
  !insertmacro uninstallTrubble ""
  !insertmacro uninstallTrubble "un."
...
;functions
  ...
  Function .onInit
    ...
    checkOther:
        ; Check for existing installation of trubble
        ReadRegStr $R0 HKLM \
  ...
  Function .onInstSuccess
    ...
    ; If StartTrubbleDelayed is 1, then set the service to start delayed
    ${If} $StartTrubbleDelayed == 1
        nsExec::Exec "nssm.exe set trubble Start SERVICE_DELAYED_AUTO_START"
    ${EndIf}

    ; If start-trubble is 1, then start the service
    ${If} $StartTrubble == 1
        nsExec::Exec 'net start trubble'
    ${EndIf}
...
;Specialty Fuctions
  Function makeUserConfig
    ...
    confFind:
	IfFileExists "$INSTDIR\etc\trubble\trubble.d\user.conf" confFound confNotFound

    confNotFound:
    ClearErrors
    FileOpen $9 "$INSTDIR\etc\trubble\trubble.d\user.conf" w
    IfErrors confReallyNotFound
        goto confLoop

    confFound:
    Delete "$INSTDIR\etc\trubble\trubble.d\user.conf"
        goto confFind

    confLoop:
    
        FileWrite $9 "trubblestack:$\r$\n"
        ...
        FileWrite $9 "        sourcetype_nova: trubble_audit$\r$\n"
        FileWrite $9 "        sourcetype_nebula: trubble_osquery$\r$\n"
        FileWrite $9 "        sourcetype_pulsar: trubble_fim$\r$\n"
        FileWrite $9 "        sourcetype_log: trubble_log$\r$\n"
            goto EndOfFile
...
Function parseCommandLineSwitches
    ...
    ; Check for start-trubble switches
    ${GetOptions} $R0 "/start-trubble=" $R2

    # Service: Start Trubble
    ${IfNot} $R2 == ""
        ; If start-trubble was passed something, then set it
        StrCpy $StartTrubble $R2
    ${ElseIfNot} $R1 == ""
        ; If start-service was passed something, then set StartTrubble to that
        StrCpy $StartTrubble $R1
    ${Else}
        ; Otherwise default to 1
        StrCpy $StartTrubble 1
    ${EndIf}

    # Service: Trubble Startup Type Delayed
    ${GetOptions} $R0 "/start-trubble-delayed" $R1
    IfErrors start_trubble_delayed_not_found
        StrCpy $StartTrubbleDelayed 1
    start_trubble_delayed_not_found:

    # Trubble Config: Token IP/Name
