# -*- coding: utf-8 -*-
'''
Main entry point for the trubble daemon
'''
...
from trubblestack.hangtime import hangtime_wrapper
...
def run():
    ...
    if __opts__['buildinfo']:
        try:
            from trubblestack import __buildinfo__
        except ImportError:
...
def getsecondsbycronexpression(base, cron_exp):
    ...
    expression provided in the trubble config
    '''
...
def getlastrunbycron(base, seconds):
    '''
    this function will use the cron_exp provided in the trubble config to
    execute the trubble processes as per the scheduled cron time
    '''
    epoch_base_datetime = time.mktime(base.timetuple())
...
def getlastrunbybuckets(buckets, seconds):
    ...
    where each bucket executes trubble processes at a different time
    '''
...
def schedule():
    ...
    function
        ...
        salt module can be run in this way, but we recommend sticking to trubble
        ...
        if 'last_run' not in jobdata:
            if jobdata.get('run_on_start', False):
                ...
                if splay:
                    ...
                    # execute the trubble process based on cron expression
...
def load_config():
    ...
    if salt.utils.platform.is_windows():
        if parsed_args.get('configfile') is None:
            parsed_args['configfile'] = 'C:\\Program Files (x86)\\Trubble\\etc\\trubble\\trubble.conf'
        salt.config.DEFAULT_MINION_OPTS['cachedir'] = 'C:\\Program Files (x86)\\trubble\\var\\cache'
        salt.config.DEFAULT_MINION_OPTS['pidfile'] = 'C:\\Program Files (x86)\\trubble\\var\\run\\trubble.pid'
        salt.config.DEFAULT_MINION_OPTS['log_file'] = 'C:\\Program Files (x86)\\trubble\\var\\log\\trubble.log'

    else:
        if parsed_args.get('configfile') is None:
            parsed_args['configfile'] = '/etc/trubble/trubble'
        salt.config.DEFAULT_MINION_OPTS['cachedir'] = '/var/cache/trubble'
        salt.config.DEFAULT_MINION_OPTS['pidfile'] = '/var/run/trubble.pid'
        salt.config.DEFAULT_MINION_OPTS['log_file'] = '/var/log/trubble'
    ...
    salt.config.DEFAULT_MINION_OPTS['default_include'] = 'trubble.d/*.conf'
    ...
    if __salt__['config.get']('splunklogging', False):
        ...
        class MockRecord(object):
            def __init__(self, message, levelname, asctime, name):
                ...
                self.name = name
        handler.emit(MockRecord(__grains__, 'INFO', time.asctime(), 'trubblestack.grains_report'))
...
#
# tag='trubble:rg' will appear in the logs to differentiate this from other
# hangtime_wrapper timers (if any)
@hangtime_wrapper(timeout=600, repeats=True, tag='trubble:rg')
def refresh_grains(initial=False):
    ...
    __opts__['trubble_uuid'] = __grains__.get('trubble_uuid', None)
    __pillar__ = {}
...
def parse_args():
    ...
    parser.add_argument('-c', '--configfile',
                        default=None,
                        help='Pass in an alternative configuration file. Default: /etc/trubble/trubble')
    ...
    parser.add_argument('--ignore_running',
                        action='store_true',
                        help='Ignore any running trubble processes. This disables the pidfile.')
...
def check_pidfile(kill_other=False):
    ...
    indicated process is alive and is Trubble.

    kill_other
        Default false, if set to true, attempt to kill detected running Trubble
            ...
            if xpid:
                ...
                if os.path.isdir("/proc/{pid}".format(pid=xpid)):
                    with open("/proc/{pid}/cmdline".format(pid=xpid),'r') as f2:
                        ...
                        if 'trubble' in cmdline:
                            if kill_other:
                                log.warn("process seems to still be alive and is trubble, attempting to shutdown")
                                ...
                                log.error("refusing to run while another trubble instance is running")
                                sys.exit(1)
                        else:
                            log.info("process does not appear to be trubble, ignoring")
