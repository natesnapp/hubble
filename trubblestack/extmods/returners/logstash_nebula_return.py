# -*- encoding: utf-8 -*-
'''
TrubbleStack Nebula-to-Logstash (http input) returner

Deliver TrubbleStack Nebula query data into Logstash using the HTTP input
                ...
                for query_name, query_results in query.iteritems():
                    for query_result in query_results['data']:
                        ...
                        try:
                            ...
                            requests.post('{}:{}/trubble/nebula'.format(indexer, port), rdy, auth=HTTPBasicAuth(user, password))
    return


def _get_options():
    if __salt__['config.get']('trubblestack:returner:logstash'):
        logstash_opts = []
        returner_opts = __salt__['config.get']('trubblestack:returner:logstash')
        ...
        for opt in returner_opts:
            ...
            processed['sourcetype'] = opt.get('sourcetype_nebula', 'trubble_osquery')
        ...
        try:
            port = __salt__['config.get']('trubblestack:returner:logstash:port')
            user = __salt__['config.get']('trubblestack:returner:logstash:user')
            indexer = __salt__['config.get']('trubblestack:returner:logstash:indexer')
            password = __salt__['config.get']('trubblestack:returner:logstash:password')
            sourcetype = __salt__['config.get']('trubblestack:nebula:returner:logstash:sourcetype')
            custom_fields = __salt__['config.get']('trubblestack:nebula:returner:logstash:custom_fields', [])
        except:
            return None

        logstash_opts = {'password': password, 'indexer': indexer, 'sourcetype': sourcetype, 'index': index, 'custom_fields': custom_fields}

        indexer_ssl = __salt__['config.get']('trubblestack:nebula:returner:logstash:indexer_ssl', True)
        logstash_opts['http_input_server_ssl'] = indexer_ssl
        logstash_opts['proxy'] = __salt__['config.get']('trubblestack:nebula:returner:logstash:proxy', {})
        logstash_opts['timeout'] = __salt__['config.get']('trubblestack:nebula:returner:logstash:timeout', 9.05)
