# -*- encoding: utf-8 -*-
'''
TrubbleStack Nebula-to-graylog (http input) returner

Deliver TrubbleStack Nebula query data into graylog using the HTTP input
            ...
            for query in data:
                for query_name, query_results in query.iteritems():
                    for query_result in query_results['data']:
                        ...
                        payload.update({'short_message': 'trubblestack'})
                        payload.update({'trubblemsg': event})
...
def _get_options():
    if __salt__['config.get']('trubblestack:returner:graylog'):
        graylog_opts = []
        returner_opts = __salt__['config.get']('trubblestack:returner:graylog')
        ...
        try:
            port = __salt__['config.get']('trubblestack:returner:graylog:port')
            gelfhttp = __salt__['config.get']('trubblestack:returner:graylog:gelfhttp')
            sourcetype = __salt__['config.get']('trubblestack:nebula:returner:graylog:sourcetype')
            custom_fields = __salt__['config.get']('trubblestack:nebula:returner:graylog:custom_fields', [])
        except:
            return None

        graylog_opts = {'gelfhttp': gelfhttp, 'sourcetype': sourcetype, 'custom_fields': custom_fields}

        gelfhttp_ssl = __salt__['config.get']('trubblestack:nebula:returner:graylog:gelfhttp_ssl', True)
        graylog_opts['http_input_server_ssl'] = gelfhttp_ssl
        graylog_opts['proxy'] = __salt__['config.get']('trubblestack:nebula:returner:graylog:proxy', {})
        graylog_opts['timeout'] = __salt__['config.get']('trubblestack:nebula:returner:graylog:timeout', 9.05)
