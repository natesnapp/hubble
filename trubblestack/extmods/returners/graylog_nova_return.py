# -*- encoding: utf-8 -*-
'''
TrubbleStack Nova-to-graylog (http input) returner

Deliver TrubbleStack Nova data into graylog using the HTTP input
...
def _get_options():
    if __salt__['config.get']('trubblestack:returner:graylog'):
        ...
        try:
            ...
            user = __salt__['config.get']('trubblestack:returner:graylog:user')
            gelfhttp = __salt__['config.get']('trubblestack:returner:graylog:gelfhttp')
            sourcetype = __salt__['config.get']('trubblestack:returner:graylog:sourcetype')
            custom_fields = __salt__['config.get']('trubblestack:returner:graylog:custom_fields', [])
        except:
            return None

        graylog_opts = {'gelfhttp': gelfhttp, 'sourcetype': sourcetype, 'custom_fields': custom_fields}

        gelfhttp_ssl = __salt__['config.get']('trubblestack:nova:returner:graylog:gelfhttp_ssl', True)
        graylog_opts['http_input_server_ssl'] = gelfhttp_ssl
        graylog_opts['proxy'] = __salt__['config.get']('trubblestack:nova:returner:graylog:proxy', {})
        graylog_opts['timeout'] = __salt__['config.get']('trubblestack:nova:returner:graylog:timeout', 9.05)
