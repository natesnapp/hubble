# -*- encoding: utf-8 -*-
'''
TrubbleStack Pulsar-to-Logstash (http input) returner

Deliver TrubbleStack Pulsar event data into Logstash using the HTTP input
...
def returner(ret):
    ...
    for opts in opts_list:
        ...
        for alert in alerts:
            ...
            requests.post('{}:{}/trubble/pulsar'.format(indexer, port), rdy, auth=HTTPBasicAuth(user, password))
    return
...
def _get_options():
    if __salt__['config.get']('trubblestack:returner:logstash'):
        ...
        try:
            ...
            sourcetype = __salt__['config.get']('trubblestack:pulsar:returner:logstash:sourcetype')
            custom_fields = __salt__['config.get']('trubblestack:pulsar:returner:logstash:custom_fields', [])
        except:
            return None

        logstash_opts = {'password': password, 'indexer': indexer, 'sourcetype': sourcetype, 'index': index, 'custom_fields': custom_fields}

        indexer_ssl = __salt__['config.get']('trubblestack:pulsar:returner:logstash:indexer_ssl', True)
        logstash_opts['http_input_server_ssl'] = indexer_ssl
        logstash_opts['proxy'] = __salt__['config.get']('trubblestack:pulsar:returner:logstash:proxy', {})
        logstash_opts['timeout'] = __salt__['config.get']('trubblestack:pulsar:returner:logstash:timeout', 9.05)
