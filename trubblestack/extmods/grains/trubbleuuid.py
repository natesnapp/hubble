...
def trubble_uuid():
    ...
    across restarts
    '''
    cached_uuid_path = os.path.join(os.path.dirname(__opts__['configfile']), 'trubble_cached_uuid')
    existing_uuid = __opts__.get('trubble_uuid', None)
    try:
        if os.path.isfile(cached_uuid_path):
            with open(cached_uuid_path, 'r') as f:
                ...
                if existing_uuid and cached_uuid != existing_uuid:
                    log.error('trubble_uuid changed on disk unexpectedly!'
                              '\nPrevious: {0}\nNew: {1}\nKeeping previous.'
                    ...
                    try:
                        ...
                        log.exception('Problem writing cached trubble uuid to file: {0}'
                                      .format(cached_uuid_path))
                    return {'trubble_uuid': existing_uuid}
                return {'trubble_uuid': cached_uuid}
        elif existing_uuid:
            log.error('trubble_uuid was previously generated, but the cached '
                      'file is no longer present: {0}'.format(cached_uuid_path))
        else:
            log.warning('generating fresh uuid, no cache file found. '
                       '(probably not a problem)')
    except Exception:
        log.exception('Problem retrieving cached trubble uuid from file: {0}'
                      .format(cached_uuid_path))
