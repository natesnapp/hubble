...
def process(configfile='salt://trubblestack_pulsar/trubblestack_pulsar_config.yaml',
            verbose=False):
    '''
    ...
    .. code-block:: yaml
        ...
        beacons:
          pulsar:
            paths:
              - /var/cache/salt/minion/files/base/trubblestack_pulsar/trubblestack_pulsar_config.yaml
    ...
    * onlydir           - Operate only if name is directory
    ...
    If pillar/grains/minion config key `trubblestack:pulsar:maintenance` is set to
    ...
    if __salt__['config.get']('trubblestack:pulsar:maintenance', False):
...
def canary(change_file=None):
    ...
    Defaults to CONF_DIR/fim_canary.tmp, i.e. /etc/trubble/fim_canary.tmp
    '''
...
def top(topfile='salt://trubblestack_pulsar/top.pulsar',
        verbose=False):
    '''
    ...
    Paths in the topfile should be relative to `salt://trubblestack_pulsar`, and
