...
Configuration:
    - trubblestack:nova:module_dir
    - trubblestack:nova:profile_dir
    - trubblestack:nova:saltenv
    - trubblestack:nova:autoload
    - trubblestack:nova:autosync
'''
...
from trubblestack.extmods.modules.nova_loader import NovaLazyLoader
...
def audit(configs=None,
    ...
    configs
        ...
        If configs is not provided, this function will call ``trubble.top``
        instead.
    ...
    verbose
        ...
        `trubblestack:nova:verbose` in minion config/pillar.

    show_success
        Whether to show successful audits in addition to failed audits.
        Defaults to True. Configurable via `trubblestack:nova:show_success` in
    ...
    show_compliance
        ...
        `trubblestack:nova:show_compliance` in minion config/pillar.
    ...
    called_from_top
        ...
        of this function and calls from trubble.top.

    debug
        Whether to log additional information to help debug nova. Defaults to
        False. Configurable via `trubblestack:nova:debug` in minion
        config/pillar.
    ...
    **kwargs
    ...
    CLI Examples::
        ...
        salt '*' trubble.audit foo
        salt '*' trubble.audit foo,bar tags='CIS*'
        salt '*' trubble.audit foo,bar.baz verbose=True
    '''
    if configs is None:
    ...
    if labels:
        if not isinstance(labels, list):
            labels=labels.split(',')
    if not called_from_top and __salt__['config.get']('trubblestack:nova:autoload', True):
        load()
    if not __nova__:
        return False, 'No nova modules/data have been loaded.'

    if verbose is None:
        verbose = __salt__['config.get']('trubblestack:nova:verbose', False)
    if show_success is None:
        show_success = __salt__['config.get']('trubblestack:nova:show_success', True)
    if show_compliance is None:
        show_compliance = __salt__['config.get']('trubblestack:nova:show_compliance', True)
    if show_profile is not None:
        log.warning(
            'Keyword argument \'show_profile\' is no longer supported'
        )
    if debug is None:
        debug = __salt__['config.get']('trubblestack:nova:debug', False)
    ...
    nova_kwargs_config = __salt__['config.get']('trubblestack:nova:nova_kwargs', False)
...
def _run_audit(configs, tags, debug, labels, **kwargs):
    ...
    if debug:
        log.debug('trubble.py configs:')
        log.debug(configs)
        log.debug('trubble.py data_list:')
        log.debug(data_list)
    # Run the audits
    ...
    if debug:
        log.debug('trubble.py control data:')
...
def top(topfile='top.nova',
    ...
    yaml file which will be run with trubble.audit. You can also specify a
    ...
    topfile
        The path of the topfile, relative to your trubblestack_nova_profiles
        directory.
    ...
    .. code-block:: bash
        ...
        salt '*' trubble.top
        salt '*' trubble.top foo/bar/top.nova
        salt '*' trubble.top foo/bar.nova verbose=True
    '''
    if __salt__['config.get']('trubblestack:nova:autoload', True):
        load()
    if not __nova__:
        return False, 'No nova modules/data have been loaded.'
...
def sync(clean=False):
    ...
    search the base environment for a top level ``trubblestack_nova``
    ...
    search the base environment for a top level ``trubblestack_nova_profiles``
    directory, unless otherwise specified via pillar or minion config
    (``trubblestack:nova:profile_dir``)
    ...
    .. code-block:: bash
        ...
        salt '*' nova.sync
        salt '*' nova.sync saltenv=trubble
    '''
    log.debug('syncing nova modules')
    nova_profile_dir = __salt__['config.get']('trubblestack:nova:profile_dir',
                                              'salt://trubblestack_nova_profiles')
    nova_module_dir, cached_profile_dir = _trubble_dir()
    saltenv = __salt__['config.get']('trubblestack:nova:saltenv', 'base')
...
def load():
    ...
    '''
    if __salt__['config.get']('trubblestack:nova:autosync', True):
        sync()

    for nova_dir in _trubble_dir():
    ...
    global __nova__
    __nova__ = NovaLazyLoader(_trubble_dir(), __opts__, __grains__, __pillar__, __salt__)
...
def _trubble_dir():
    ...
    nova_profile_dir = __salt__['config.get']('trubblestack:nova:profile_dir',
                                              'salt://trubblestack_nova_profiles')
    nova_module_dir = os.path.join(__opts__['install_dir'], 'files', 'trubblestack_nova')
    # Support optional salt:// in config
    if 'salt://' in nova_profile_dir:
        _, _, nova_profile_dir = nova_profile_dir.partition('salt://')
    saltenv = __salt__['config.get']('trubblestack:nova:saltenv', 'base')
...
def _get_top_data(topfile):
    ...
    '''
    topfile = os.path.join(_trubble_dir()[1], topfile)
