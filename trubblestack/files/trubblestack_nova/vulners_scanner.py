'''
TrubbleStack Nova plugin for auditing installed packages.
...
This audit module requires a YAML file inside the trubblestack_nova_profiles directory.
...
def _process_vulners(vulners):
    '''
    Process the data returned by the API into the format accepted by `trubble.py`.

    :param vulners: The JSON data returned by the API
    :return: A list of dictionaries as trubble.py swallows
    '''
