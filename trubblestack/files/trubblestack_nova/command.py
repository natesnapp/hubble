# -*- encoding: utf-8 -*-
'''
Trubble Nova plugin for running arbitrary commands and checking the output of
those commands

This module is deprecated, and must be explicitly enabled in pillar/minion
config via the trubblestack:nova:enable_command_module (should be set to True
...
def audit(data_list, tags, labels, **kwargs):
    ...
    if __tags__ and not __salt__['config.get']('trubblestack:nova:enable_command_module',
                                               False):
        ret['Errors'] = ['command module has not been explicitly enabled in '
                         'config. Please set trubblestack:nova:enable_command_module '
