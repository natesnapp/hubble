# -*- encoding: utf-8 -*-
'''
TrubbleStack Nova module for using stat to verify ownership & permissions.
            ...
            for tag_data in __tags__[tag]:
                ...
                if 'allow_more_strict' in expected.keys() and 'mode' not in expected.keys():
                    ...
                    reason = "'allow_more_strict' tag can't be specified without 'mode' tag." \
                             " Seems like a bug in trubble profile."
                ...
                if not salt_ret:
                    if not expected:
                        ...
                        tag_data['failure_reason'] = "Could not get access any file at '{0}'. " \
                                                     "File might not exist, or trubble might not" \
                ...
                for e in expected.keys():
                    ...
                    if e == 'mode':
                        ...
                        if not isinstance(allow_more_strict, bool):
                            passed = False
                            reason = "{0} is not a valid boolean. Seems like a bug in trubble profile." \
