...
import trubblestack.files.trubblestack_nova.pkg
...
class TestPkg():
    ...
    def test_virtual(self):
        expected_val = True
        val = trubblestack.files.trubblestack_nova.pkg.__virtual__()
    ...
    def test_merge_yaml(self):
        ...
        val = trubblestack.files.trubblestack_nova.pkg._merge_yaml(ret, data, profile)
    ...
    def test_audit_for_success(self):
        ...
        __tags__ = 'CIS-6.5'
        trubblestack.files.trubblestack_nova.pkg.__grains__ = {'osfinger': 'Ubuntu-16.04'}
        __salt__ = {}

        def pkg_version(name):
            return name
        __salt__['pkg.version'] = pkg_version
        trubblestack.files.trubblestack_nova.pkg.__salt__ = __salt__
        val = trubblestack.files.trubblestack_nova.pkg.audit(data_list, __tags__, debug=False)
    ...
    def test_get_tags(self):
        ...
        data = {'pkg':
                {'blacklist': [{'talk1': {'nova_profile': 'ubuntu-1604-level-1-scored-v1-0-0', 'data': {'Ubuntu-16.04': [{'/etc/inetd.conf': {'pattern': '^talk', 'tag': 'CIS-5.1.4'}}, {'/etc/inetd.conf': {'pattern': '^ntalk', 'tag': 'CIS-5.1.4'}}]}, 'description': 'Ensure talk server is not enabled'}},
                 ...
                 'whitelist': [{'ssh_ignore_rhosts': {'nova_profile': 'ubuntu-1604-level-1-scored-v1-0-0', 'data': {'Ubuntu-16.04': [{'/etc/ssh/sshd_config': {'pattern': 'IgnoreRhosts', 'tag': 'CIS-9.3.6', 'match_output': 'yes'}}]}, 'description': 'Set SSH IgnoreRhosts to Yes'}}]}}
        val = trubblestack.files.trubblestack_nova.pkg._get_tags(data)
    ...
    def test_get_tags_for_empty_data(self):
        data = {'pkg': {}}
        trubblestack.files.trubblestack_nova.pkg.__grains__ = {'osfinger': 'Ubuntu-16.04'}
        ret = trubblestack.files.trubblestack_nova.pkg._get_tags(data)
        assert ret == {}
